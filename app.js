...
async function initApp() {
  ...
  // Dùng token để ping repo — hỗ trợ cả repo private
  const headers = { 'Accept': 'application/vnd.github+json' };
  if (cfg.token) headers['Authorization'] = `Bearer ${cfg.token}`;

  try {
    const res = await fetch(`${GH_API}/repos/${cfg.username}/${cfg.repo}`, { headers });

    if (res.status === 401 || res.status === 403) {
      // Token hết hạn — cảnh báo nhưng vẫn tiếp tục (repo có thể public)
      setStatus('warn', `${cfg.username}/${cfg.repo} — Token hết hạn`);
      toast('⚠️ Token hết hạn — chỉ xem được, không upload', 'warn');
      loadFiles();
      return;
    }
    ...
    if (!res.ok) {
      ...
      toast('Không tìm thấy repo. Nhấn ⚙️ để cấu hình lại.', 'error');
      showModal('config-modal');
      return;
    }
    ...
    toast('Lỗi kết nối — kiểm tra internet', 'error');
  }
}
...
async function saveConfig() {
  ...
  // 1. Verify token trước (GET /user) — hoạt động dù repo public hay private
  ...
  if (!tokenRes || !tokenRes.ok) {
    ...
    const status = tokenRes ? tokenRes.status : 'network error';
    return toast(`Token không hợp lệ (${status}) — tạo token mới tại github.com/settings/tokens`, 'error');
  }

  // 2. Verify token có quyền truy cập repo không (dùng token để GET repo)
  ...
  if (!repoRes || !repoRes.ok) {
    ...
    if (repoRes && repoRes.status === 404) {
      return toast(`Không tìm thấy repo "${u}/${r}" — kiểm tra tên user và repo`, 'error');
    }
    return toast(`Lỗi truy cập repo (${repoRes?.status}) — kiểm tra token có quyền "repo" không`, 'error');
  }
