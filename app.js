...
async function pingGitHub() {
  try { return (await ghFetch(`/repos/${cfg.username}/${cfg.repo}`)).ok; }
  catch { return false; }
}
...
function renderFiles(files) {
  ...
  for (const [label, group] of Object.entries(groups)) {
    ...
    group.forEach(f => {
      ...
      const fid    = btoa(encodeURIComponent(f.name)).replace(/[+=\/]/g,'_');
      ...
      html += `
        <div class="file-card" id="fc-${fid}" data-name="${escAttr(f.name)}">
          ...
          <button class="file-dl-btn" onclick="downloadFile(${JSON.stringify(rawUrl)},${JSON.stringify(f.name)})" title="Tải xuống">⬇</button>
        </div>`;
    });
  }
